...
import { TypeList } from "@/types/filter";
  ...
  return (
    <div className="min-h-screen bg-slate-900 px-3 sm:px-4 lg:px-6 py-20 text-white">
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 mb-6">
        <select
          value={typeList}
          onChange={(e) => setTypeList(e.target.value as TypeList)}
          className="bg-slate-800 p-2 rounded overflow-y-auto  scrollbar-thumb-green-500 scrollbar-track-gray-800 scrollbar-thin"
        >
          {Object.entries(typeListLabels).map(([key, label]) => (
            <option
              key={key}
              value={key}
              className="hover:bg-lime-400 bg-slate-700"
            >
              {label}
            </option>
          ))}
        ...
        <select
          value={sortLang}
          onChange={(e) => setSortLang(e.target.value as LangList)}
          className="bg-slate-800 p-2 rounded overflow-y-auto  scrollbar-thumb-green-500 scrollbar-track-gray-800 scrollbar-thin"
        >
          {sortLangs.map((lang) => (
